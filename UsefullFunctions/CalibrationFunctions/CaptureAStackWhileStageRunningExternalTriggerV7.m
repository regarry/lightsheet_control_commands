...
function [] = CaptureAStackWhileStageRunningExternalTriggerV7(controlParameters, allLasers,...
    ...
    , stepSize, pathToSave, tifFormat, delayImages, numOfZeroImagesBegin, numOfZeroImagesEnd, downSampleFactor)
    ...
    confidenceMargin = 0.2; %Make sure we do not move the objective above this value
    ...
    %Put the zero images now, to compensate for different thickness of the
    %tissue
    for ii = 1:numOfZeroImagesBegin
        
        filePrefixStr = num2str((ii)*stepSize*10);
        filePrefix = '000000';
        ...
        img = zeros(height/downSampleFactor, width/downSampleFactor); % image should be interpreted as a 2D array
        img = uint16(img);
        if (tifFormat)
...
%     end
    ...
    img = rot90(img);                % make column-major order for MATLAB
    ...
    for ii = 1:(numOfImages + delayImages)
        ...
        if (tifFormat)
            ...
            if (ii > delayImages) 
                ...
                filePrefixStr = num2str((ii - delayImages + numOfZeroImagesBegin)*stepSize*10);
                filePrefix = '000000';
                switch numel(filePrefixStr)
                    case 1 
                        filePrefix(6) = filePrefixStr;
        ...
        if (ii + delayImages + controlParameters.MMC.getRemainingImageCount() > frameNumberToMoveVector(indexF
            .orObjectiveCorrection))
            ...
            deltaForDet = absoluteMovementVector(indexForObjectiveCorrection - 1) - str2num(curPosDet)
    ...
    %Put zero images at the end of the folder to compensate for various
    %thicknesses
     for ii = (numOfImages + numOfZeroImagesBegin + 1):(numOfImages + numOfZeroImagesBegin + numOfZeroImagesEnd)
