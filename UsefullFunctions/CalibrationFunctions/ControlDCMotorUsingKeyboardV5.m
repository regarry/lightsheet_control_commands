...
function [] = ControlDCMotorUsingKeyboardV5(sExtLens, sDetLens, sFWInt, mmc, cameraLabel, frameRate, expTime, addLine)
    ...
    %Check the position of the filter wheel
    [posIntFilter] = GetFilterPosition(sFWInt);
    ...
    'String',['Pos ext = ',posExt,', Step ext = 0.1 mm, Pos det = ',posDet,', Step det = 0.01, exp time =',char(expVal),' Intensity filter pos = ',posIntFilter],...
    ...
    myhandles.posIntFilter = posIntFilter;
    myhandles.sFWInt = sFWInt;
    myhandles.mmc = mmc;
...
function [] = KeyReleaseFcn(src, evt)
        ...
        [positionFW] = GetFilterPosition(myhandles.sFWInt);
        switch lower(evt.Key)
            ...
            case 'e'
                ...
                case 'u' 
                [positionFW] = GetFilterPosition(myhandles.sFWInt);
                myhandles.posIntFilter = str2num(positionFW);
                if (myhandles.posIntFilter < 12)
                           myhandles.posIntFilter = myhandles.posIntFilter + 1;             
                           SetFilterPosition(myhandles.sFWInt, myhandles.posIntFilter);
                end
                [positionFW] = GetFilterPosition(myhandles.sFWInt);
            case 'y'
                [positionFW] = GetFilterPosition(myhandles.sFWInt);
                myhandles.posIntFilter = str2num(positionFW);
                if (myhandles.posIntFilter > 1)
                           myhandles.posIntFilter = myhandles.posIntFilter - 1;             
                           SetFilterPosition(myhandles.sFWInt, myhandles.posIntFilter);
                end                                
                [positionFW] = GetFilterPosition(myhandles.sFWInt);
        ...
        textToDisplay = ['Pos Ext = ',posExt,'   Step Ext = ',myhandles.stepSizeExtOpt{myhandles.stepSizeExtIndex},...
            '   Pos Det = ',posDet,'   Step Det = ',myhandles.stepSizeDetOpt{myhandles.stepSizeDetIndex},'  Exp time = ',char(expVal)...
            ,'ms Intensity filter pos = ',positionFW];
