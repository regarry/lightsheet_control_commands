...
%controlParameters.sExtLens - a serial port to the detection objective
...
function [bestExcitationLensPos, imageAtbestExcitationLensPos, actualScanLocations, qualityMeasureVector] = CenterTheLightSheetV3(...
    ...
    if (relativePositionRangeUm < 0)        
        [bestExcitationLensPos] = str2double(GetPos(controlParameters.sExtLens)); 
    ...
    [curLoc] = GetPos(controlParameters.sExtLens);
    ...
    fprintf(controlParameters.sExtLens,['1PR-',num2str(relativePositionRangeMm)]);
    ...
    fprintf(controlParameters.sExtLens,['1VA',stageSpeedDuringScan]); 
    ...
    for ii = 1:numel(scanVectorMm)
        ...
        [actualScanLocations(ii)] = str2double(GetPos(controlParameters.sExtLens)); %The location is a string
...
%             end 
        ...
        fprintf(controlParameters.sExtLens,['1PR',num2str(scanResMm)]);
...
%     end
    ...
    if (moveToBestFocalPoint)
         ...
         pause(1);
         fprintf(controlParameters.sExtLens,['1PR',num2str(diff)]);         
    end  
    
    %Check and close the shutter
    if (controlParameters.MMC.isSequenceRunning)
