...
function [relativeMovementVector, timePointsVector, frameNumberToMoveVector] = CreateADetLensCalibCurveV3(scanCalibrationParameters, frameRate, ...
...
%     end
    ...
    for ii = 1:(numel(detLensVector) - 1)
        ...
        if (slop >= 0)
             ...
             numOfMovementPointsInSegment = abs(ceil((detLensVector(ii+1) - detLensVector(ii))/(toleranceInFocusMm))) + 1;
        end
        ...
        for jj = 1:numOfMovementPointsInSegment
            addVectorRelativeLocation(jj) = detLensVector(ii) + sign(slop)*toleranceInFocusMm*(jj-1);
            if (slop ~= 0)
                timeValue = toleranceInFocusMm*(jj-1)/abs(slop);
