...
%controlParameters.sDetLens - a serial port to the detection objective
%controlParameters.cameraLabel - the MM label of the camera
...
function [bestFocusPos, imageAtBestFocusPos, actualScanLocations, qualityMeasureVector] = FocusDetectionObjectiveV3(...
    relativePositionRangeUm, scanResUm, whichQualityMeasure, aveNum, ...
    ...
    controlParameters.MMC.setCircularBufferMemoryFootprint(1024*4);     
    width = controlParameters.MMC.getImageWidth();
    height = controlParameters.MMC.getImageHeight();
    downSampleFactor = 2048/width;
    pixelType = 'uint16';
    
    %Turn off the power supply if wuality measure is one
    if (whichQualityMeasure == 1)
        fwrite(controlParameters.afg, ':output1 off;');
    end
    ...
    [curLoc] = GetPos(controlParameters.sDetLens);
    display(['Stage start position is:',curLoc]);  
    fprintf(controlParameters.sDetLens,'1VA0.25'); 
    fprintf(controlParameters.sDetLens,['1PR-',num2str(relativePositionRangeMm)]);
    ...
    for ii = 1:numel(scanVectorMm)
        ...
        [actualScanLocations(ii)] = str2double(GetPos(controlParameters.sDetLens)); %The location is a string
        ...
        for jj = 1:aveNum            
            ...
            startTime = tic;
            if (controlParameters.MMC.getRemainingImageCount < 1)
                pause(0.2);
            end
...
%             end            
        ...
        fprintf(controlParameters.sDetLens,['1PR',num2str(scanResMm)]);
        ...
        switch whichQualityMeasure
            ...
            case 4
                [qualityMeasureVector(ii)] = TamuraCoeeficient(aveImage, widthOfWindow, heightOfWindow);
         end
...
function [qualityMeasure] = EvaluateFWHM(image, lengthFromCenterInPixels, verPosLightSheet, numOfSections, spaceBetweenSectionsInPixels, showImages)
    ...
    for ii = 1:numOfSections
        ...
        c = image(crossSectionCordV , crossSectionCordH(ii)+ (-widthForAverage:widthForAverage));
        c = sum(c');
...
function [qualityMeasure] = TamuraCoeeficient(aveImage, widthOfWindow, heightOfWindow)
    ...
    qualityMeasure = sqrt(std(temp(:))/mean(temp(:)));
